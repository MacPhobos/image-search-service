...
class TestIsRqWorker:
    """Tests for is_rq_worker() function."""

    @patch("multiprocessing.current_process")
    def test_main_process_not_worker(self, mock_process: MagicMock) -> None:
        """Should return False in main process."""
        from image_search_service.core.device import is_rq_worker

        mock_proc = MagicMock()
        mock_proc.name = "MainProcess"
        mock_process.return_value = mock_proc

        assert is_rq_worker() is False

    @patch("multiprocessing.current_process")
    def test_process_dash_number_is_worker(self, mock_process: MagicMock) -> None:
        """Should return True for Process-N naming pattern."""
        from image_search_service.core.device import is_rq_worker

        mock_proc = MagicMock()
        mock_proc.name = "Process-1"
        mock_process.return_value = mock_proc

        assert is_rq_worker() is True

    @patch("multiprocessing.current_process")
    def test_fork_process_is_worker(self, mock_process: MagicMock) -> None:
        """Should return True for ForkProcess-N naming pattern."""
        from image_search_service.core.device import is_rq_worker

        mock_proc = MagicMock()
        mock_proc.name = "ForkProcess-2"
        mock_process.return_value = mock_proc

        assert is_rq_worker() is True

    @patch("multiprocessing.current_process")
    def test_spawn_process_is_worker(self, mock_process: MagicMock) -> None:
        """Should return True for SpawnProcess-N naming pattern."""
        from image_search_service.core.device import is_rq_worker

        mock_proc = MagicMock()
        mock_proc.name = "SpawnProcess-3"
        mock_process.return_value = mock_proc

        assert is_rq_worker() is True

    @patch("multiprocessing.current_process")
    def test_custom_name_not_worker(self, mock_process: MagicMock) -> None:
        """Should return False for custom process names."""
        from image_search_service.core.device import is_rq_worker

        mock_proc = MagicMock()
        mock_proc.name = "MyCustomWorker"
        mock_process.return_value = mock_proc

        assert is_rq_worker() is False

    @patch("multiprocessing.current_process")
    def test_exception_returns_false(self, mock_process: MagicMock) -> None:
...
class TestRqWorkerMpsDisabled:
    """Tests for MPS disabled in RQ worker subprocess."""
    ...
    @patch("torch.cuda.is_available", return_value=False)
    @patch("torch.backends.mps.is_available", return_value=True)
    def test_rq_worker_on_apple_silicon_uses_cpu(
        self,
        mock_mps: MagicMock,
        mock_cuda: MagicMock,
        ...
        """Should return CPU when in RQ worker on Apple Silicon with MPS available."""
        from image_search_service.core.device import clear_device_cache, get_device

        # Simulate RQ worker subprocess
        mock_proc = MagicMock()
        mock_proc.name = "Process-1"
        mock_process.return_value = mock_proc
    ...
    @patch("torch.cuda.is_available", return_value=False)
    @patch("torch.backends.mps.is_available", return_value=True)
    def test_main_process_on_apple_silicon_uses_mps(
        self,
        mock_mps: MagicMock,
        mock_cuda: MagicMock,
        mock_machine: MagicMock,
        ...
        """Should return MPS when in main process on Apple Silicon."""
        from image_search_service.core.device import clear_device_cache, get_device
    ...
    @patch.dict(os.environ, {"DEVICE": "mps"}, clear=True)
    @patch("multiprocessing.current_process")
    @patch("platform.system", return_value="Darwin")
    @patch("platform.machine", return_value="arm64")
    def test_device_override_still_works_in_worker(
        self,
        mock_machine: MagicMock,
        mock_system: MagicMock,
        mock_process: MagicMock,
    ) -> None:
        """Should respect DEVICE override even in RQ worker."""
        from image_search_service.core.device import clear_device_cache, get_device
        ...
        # DEVICE env var takes precedence
        assert device == "mps"
    ...
    def test_rq_worker_non_apple_silicon_uses_cpu(
        self,
        mock_mps: MagicMock,
        mock_cuda: MagicMock,
        mock_machine: MagicMock,
        ...
        """Should return CPU when in RQ worker on non-Apple Silicon."""
        from image_search_service.core.device import clear_device_cache, get_device
